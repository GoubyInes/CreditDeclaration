...
def create_xml():
        ...
        client_list = [particulier_list, entrepreneur_list, entreprise_list]

        crem = ET.Element("crem", {"xmlns:xsi": "http://www.w3.org/2001/XMLSchema-instance", "c1": "1.0"})
        ...
        for i,list_type in enumerate(client_list):
          if(i==0):
            code="001"
          if(i==1):
            code="002"
          ...
          for _,row in list_type.iterrows(): 
            ...
            if(code=="003"): #Entreprise Associe
              entreprise_associe_list= fetch_entreprise_associe(row['client_radicale'])
              ...
              #Entreprise Societe
              entreprise_societe_list= fetch_entreprise_associe(row['client_radicale'])
              ...
              entreprise_dirigeant_list= fetch_entreprise_dirigeant(row['client_radicale'])
...
def fetch_entreprise_associe(id):
    hook = MsSqlHook(mssql_conn_id='sqlserver')
    records = hook.get_pandas_df(sql="SELECT * FROM Entreprise_Associe where client_radicale= %s",parameters=(id,)) # _Associe
    return records

def fetch_entreprise_dirigeant(id):
    hook = MsSqlHook(mssql_conn_id='sqlserver')
    records = hook.get_pandas_df(sql="SELECT * FROM Entreprise_Dirigeant where client_radicale=%s",parameters=(id,)) #_Dirigeant
    return records

def fetch_entreprise_societe(id):
    hook = MsSqlHook(mssql_conn_id='sqlserver')
    records = hook.get_pandas_df(sql="SELECT * FROM Entreprise_Societe where client_radicale=%s",parameters=(id,)) #_Societe 
    return records
