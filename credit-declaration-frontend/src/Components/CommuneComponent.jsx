...
const CommuneComponent = () => {
  ...
  //call READ hook
  const {data: allCommunes = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Communes'],
    queryFn: CommuneService.getCommunes,
  });
  ...
  useEffect(() => {
    if (allCommunes?.length) {
      setTotalRowCount(allCommunes.length);
      const paginated = allCommunes.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allCommunes, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Commune) =>{ return await CommuneService.createCommune(Commune)},
        onMutate: async (Commune) => {
          await queryClient.cancelQueries(['Communes']);
          const previousCommunes = queryClient.getQueryData(['Communes']);
          queryClient.setQueryData(['Communes'], (prev) => [
            ...(prev || []),
            {...Commune},
          ]); 
          return { previousCommunes };
        },
        onSuccess: () => queryClient.invalidateQueries(['Communes']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, oldDomaine, data }) => {  await CommuneService.updateCommune(oldCode, oldDomaine, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Communes'], (prev) =>
        prev?.map((prev) =>
      ...
      onSuccess: () => queryClient.invalidateQueries(['Communes']),
    });
  }
  ...
  function useDelete() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, oldDomaine}) => {await  CommuneService.deleteCommune(oldCode, oldDomaine)},
      onMutate: async (code) => {
        queryClient.setQueryData(['Communes'], (prev) =>
          prev?.filter((p) => p.code !== code)
      )},
      onError: (err, code, context) => {if (context?.previousCommunes) queryClient.setQueryData(['Communes'], context.previousCommunes); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Communes']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Communes']),
    });
  }
  //CREATE action
  ...
  const handleSave = async ({ values, table, row }) => {
    ...
    const oldDomaine = row.original.domaine;
    await update({oldCode, oldDomaine, data: values });
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle commune</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier commune</DialogTitle>
...
function validate(Commune) {
  return {
    code: !validateRequired(Commune.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(Commune.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(Commune.domaine) ? 'Domaine est requis' : '',
  };
}
  ...
  return (
    <div>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          Voulez-vous vraiment supprimer la commune<strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
