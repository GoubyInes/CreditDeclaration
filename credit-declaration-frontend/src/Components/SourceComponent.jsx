...
const SourceComponent = () => {
  ...
  //call READ hook
  const {data: allSources = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Sources'],
  ...
  useEffect(() => {
    if (allSources?.length) {
      setTotalRowCount(allSources.length);
      const paginated = allSources.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allSources, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Source) =>{ return await SourceService.createSource(Source)},
        onMutate: async (Source) => {
          await queryClient.cancelQueries(['Sources']);
          const previousSources = queryClient.getQueryData(['Sources']);
          queryClient.setQueryData(['Sources'], (prev) => [
            ...(prev || []),
            {...Source},
          ]); 
          return { previousSources };
        },
        onSuccess: () => queryClient.invalidateQueries(['Sources']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await SourceService.updateSource(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Sources'], (prev) =>
        prev?.map((prev) =>
  ...
  function useDelete() {
    ...
    return useMutation({
      ...
      onError: (err, code, context) => {if (context?.previousSources) queryClient.setQueryData(['Sources'], context.previousSources); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Sources']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Sources']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle source</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier source</DialogTitle>
...
function validate(Source) {
  return {
    code: !validateRequired(Source.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(Source.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(Source.domaine) ? 'Domaine est requis' : '',
  };
}

  return (
    <div>
      <h2>Liste des Sources Information</h2>
      <MaterialReactTable table={table} />

      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        <DialogTitle>Confirmer la suppression</DialogTitle>
        <DialogContent>
          Voulez-vous vraiment supprimer la source d'information <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
