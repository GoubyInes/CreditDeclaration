...
import CountryService from '../Services/PaysService';
...
const CountryComponent = () => {
  ...
  //call READ hook
  const {data: allCountries = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Countries'],
  ...
  useEffect(() => {
    if (allCountries?.length) {
      setTotalRowCount(allCountries.length);
      const paginated = allCountries.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allCountries, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Country) =>{ return await CountryService.createCountry(Country)},
        onMutate: async (Country) => {
          await queryClient.cancelQueries(['Countries']);
          const previousCountries = queryClient.getQueryData(['Countries']);
          queryClient.setQueryData(['Countries'], (prev) => [
            ...(prev || []),
            {...Country},
          ]); 
          return { previousCountries };
        },
        onSuccess: () => queryClient.invalidateQueries(['Countries']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await CountryService.updateCountry(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Countries'], (prev) =>
        prev?.map((prev) =>
  ...
  function useDelete() {
    ...
    return useMutation({
      ...
      onError: (err, code, context) => {if (context?.previousCountries) queryClient.setQueryData(['Countries'], context.previousCountries); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Countries']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Countries']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouveau pays</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier pays</DialogTitle>
...
function validate(Country) {
  return {
    code: !validateRequired(Country.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(Country.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(Country.domaine) ? 'Domaine est requis' : '',
  };
}
  ...
  return (
    <div>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          Voulez-vous vraiment supprimer le pays <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
