...
const StatusComponent = () => {
  ...
  //call READ hook
  const {data: allStatuss = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Statuss'],
  ...
  useEffect(() => {
    if (allStatuss?.length) {
      setTotalRowCount(allStatuss.length);
      const paginated = allStatuss.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allStatuss, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Status) =>{ return await StatusService.createStatus(Status)},
        onMutate: async (Status) => {
          await queryClient.cancelQueries(['Statuss']);
          const previousStatuss = queryClient.getQueryData(['Statuss']);
          queryClient.setQueryData(['Statuss'], (prev) => [
            ...(prev || []),
            {...Status},
          ]); 
          return { previousStatuss };
        },
        onSuccess: () => queryClient.invalidateQueries(['Statuss']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await StatusService.updateStatus(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Statuss'], (prev) =>
        prev?.map((prev) =>
      ...
      onSuccess: () => queryClient.invalidateQueries(['Statuss']),
    });
  }
  ...
  function useDelete() {
    ...
    return useMutation({
      ...
      onError: (err, code, context) => {if (context?.previousStatuss) queryClient.setQueryData(['Statuss'], context.previousStatuss); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Statuss']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Statuss']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle état</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier état</DialogTitle>
...
function validate(Status) {
  return {
    code: !validateRequired(Status.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(Status.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(Status.domaine) ? 'Domaine est requis' : '',
  };
}

  return (
    <div>
      <h2>Liste des Etats civil</h2>
      <MaterialReactTable table={table} />

      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        <DialogTitle>Confirmer la suppression</DialogTitle>
        <DialogContent>
          Voulez-vous vraiment supprimer l'état <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
