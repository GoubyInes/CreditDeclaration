...
const DurationComponent = () => {
  ...
  //call READ hook
  const {data: allDurations = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Durations'],
    queryFn: DurationService.getDurations,
  });
  ...
  useEffect(() => {
    if (allDurations?.length) {
      setTotalRowCount(allDurations.length);
      const paginated = allDurations.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allDurations, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Duration) =>{ return await DurationService.createDuration(Duration)},
        onMutate: async (Duration) => {
          await queryClient.cancelQueries(['Durations']);
          const previousDurations = queryClient.getQueryData(['Durations']);
          queryClient.setQueryData(['Durations'], (prev) => [
            ...(prev || []),
            {...Duration},
          ]); 
          return { previousDurations };
        },
        onSuccess: () => queryClient.invalidateQueries(['Durations']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await DurationService.updateDuration(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Durations'], (prev) =>
        prev?.map((prev) =>
  ...
  function useDelete() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: DurationService.deleteDuration,
      onMutate: async (code) => {
        queryClient.setQueryData(['Durations'], (prev) =>
          prev?.filter((p) => p.code !== code)
      )},
      onError: (err, code, context) => {if (context?.previousDurations) queryClient.setQueryData(['Durations'], context.previousDurations); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Durations']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Durations']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle durée</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier durée</DialogTitle>
...
function validate(Duration) {
  return {
    code: !validateRequired(Duration.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(Duration.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(Duration.domaine) ? 'Domaine est requis' : '',
  };
}

  return (
    <div>
      <h2>Liste des Durées Crédit</h2>
      <MaterialReactTable table={table} />

      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        <DialogTitle>Confirmer la suppression</DialogTitle>
        <DialogContent>
          Voulez-vous vraiment supprimer la durée <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
