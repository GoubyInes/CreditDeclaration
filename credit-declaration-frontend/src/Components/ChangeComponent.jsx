...
const ChangeComponent = () => {
  ...
  //call READ hook
  const {data: allChanges = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Changes'],
  ...
  useEffect(() => {
    if (allChanges?.length) {
      setTotalRowCount(allChanges.length);
      const paginated = allChanges.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allChanges, pagination]);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        muiEditTextFieldProps: {
          required: true,
          error: !!validationErrors?.devise,
          helperText: validationErrors?.devise,
      ...
      },
      {
        ...
        muiEditTextFieldProps: {
          required: true,
          error: !!validationErrors?.entite,
          helperText: validationErrors?.entite,
          //remove any previous validation errors when user focuses on the input
          onFocus: () =>
            setValidationErrors({
              ...validationErrors,
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Change) =>{ return await ChangeService.createChange(Change)},
        onMutate: async (Change) => {
          await queryClient.cancelQueries(['Changes']);
          const previousChanges = queryClient.getQueryData(['Changes']);
          queryClient.setQueryData(['Changes'], (prev) => [
            ...(prev || []),
            {...Change},
          ]); 
          return { previousChanges };
        },
        onSuccess: () => queryClient.invalidateQueries(['Changes']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await ChangeService.updateChange(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Changes'], (prev) =>
        prev?.map((prev) =>
  ...
  function useDelete() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: ChangeService.deleteChange,
      onMutate: async (code) => {
        queryClient.setQueryData(['Changes'], (prev) =>
          prev?.filter((p) => p.code !== code)
      )},
      onError: (err, code, context) => {if (context?.previousChanges) queryClient.setQueryData(['Changes'], context.previousChanges); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Changes']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Changes']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle monnaie</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier monnaie</DialogTitle>
...
function validate(Change) {
  return {
    code: !validateRequired(Change.code)? 'Code est requis': '',
    devise: !validateRequired(Change.devise) ? 'Devise est requis' : '',
    entite: !validateRequired(Change.entite) ? 'Entite est requis' : '',
  };
}
  ...
  return (
    <div>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          Voulez-vous vraiment supprimer la monnaie <strong>{selectedRowToDelete?.original?.devise}</strong> ?
        </DialogContent>
        <DialogActions>
