...
const ActivityComponent = () => {
  ...
  //call READ hook
  const {data: allActivities = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['activities'],
  ...
  useEffect(() => {
    if (allActivities?.length) {
      setTotalRowCount(allActivities.length);
      const paginated = allActivities.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allActivities, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(activite) =>{ return await ActivityService.createActivity(activite)},
        onMutate: async (activite) => {
          await queryClient.cancelQueries(['activites']);
          const previousactivites = queryClient.getQueryData(['activites']);
          queryClient.setQueryData(['activites'], (prev) => [
            ...(prev || []),
            {...activite},
          ]); 
          return { previousactivites };
        },
        onSuccess: () => queryClient.invalidateQueries(['activites']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await ActivityService.updateActivity(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['activites'], (prev) =>
        prev?.map((prev) =>
      ...
      onSuccess: () => queryClient.invalidateQueries(['activites']),
    });
  }
  ...
  function useDelete() {
    ...
    return useMutation({
      ...
      onError: (err, code, context) => {if (context?.previousactivites) queryClient.setQueryData(['activites'], context.previousactivites); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['activites']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['activites']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle activité</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier activité</DialogTitle>
  ...
  function validate(activite) {
    return {
      code: !validateRequired(activite.code)
        ? 'Code est requis'
        : '',
      descriptif: !validateRequired(activite.descriptif) ? 'Descriptif est requis' : '',
      domaine: !validateRequired(activite.domaine) ? 'Domaine est requis' : '',
    };
  }
  ...
  return (
    <div>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          Voulez-vous vraiment supprimer l'activité' <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
