...
import EntityService from '../Services/EntityPublicService';
...
const EntityPublicComponent = () => {
  const [validationErrors, setValidationErrors] = useState({});
  const [data, setData] = useState([]);
  //call READ hook
  const {data: allEntities = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['entities'],
  ...
  useEffect(() => {
    if (allEntities?.length) {
      setTotalRowCount(allEntities.length);
      const paginated = allEntities.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allEntities, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(entity) =>{ return await EntityService.createEntity(entity)},
        onMutate: async (entity) => {
          await queryClient.cancelQueries(['entities']);
          const previousentities = queryClient.getQueryData(['entities']);
          queryClient.setQueryData(['entities'], (prev) => [
            ...(prev || []),
            {...entity},
          ]); 
          return { previousentities };
        },
        onSuccess: () => queryClient.invalidateQueries(['entities']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await EntityService.updateEntity(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['entities'], (prev) =>
        prev?.map((prev) =>
  ...
  function useDelete() {
    ...
    return useMutation({
      ...
      onError: (err, code, context) => {if (context?.previousentities) queryClient.setQueryData(['entities'], context.previousentities); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['entities']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['entities']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle entité</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier entité</DialogTitle>
...
function validate(entity) {
  return {
    code: !validateRequired(entity.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(entity.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(entity.domaine) ? 'Domaine est requis' : '',
  };
}

  return (
    <div>
      <h2>Liste des Entités Publiques</h2>
      <MaterialReactTable table={table} />

      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        <DialogTitle>Confirmer la suppression</DialogTitle>
        <DialogContent>
          Voulez-vous vraiment supprimer l'entité <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
...
export default EntityPublicComponent;