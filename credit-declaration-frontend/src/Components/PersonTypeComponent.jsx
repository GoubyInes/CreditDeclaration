...
const PersonTypeComponent = () => {
  const [validationErrors, setValidationErrors] = useState({});
  const [data, setData] = useState([]);
  //call READ hook
  const {data: allPersonTypes = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['PersonTypes'],
    queryFn: PersonTypeService.getPersonTypes,
  });
  ...
  useEffect(() => {
    if (allPersonTypes?.length) {
      setTotalRowCount(allPersonTypes.length);
      const paginated = allPersonTypes.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allPersonTypes, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(PersonType) =>{ return await PersonTypeService.createPersonType(PersonType)},
        onMutate: async (PersonType) => {
          await queryClient.cancelQueries(['PersonTypes']);
          const previousPersonTypes = queryClient.getQueryData(['PersonTypes']);
          queryClient.setQueryData(['PersonTypes'], (prev) => [
            ...(prev || []),
            {...PersonType},
          ]); 
          return { previousPersonTypes };
        },
        onSuccess: () => queryClient.invalidateQueries(['PersonTypes']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await PersonTypeService.updatePersonType(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['PersonTypes'], (prev) =>
        prev?.map((prev) =>
      ...
      onSuccess: () => queryClient.invalidateQueries(['PersonTypes']),
    });
  }
  ...
  function useDelete() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: PersonTypeService.deletePersonType,
      onMutate: async (code) => {
        queryClient.setQueryData(['PersonTypes'], (prev) =>
          prev?.filter((p) => p.code !== code)
      )},
      onError: (err, code, context) => {if (context?.previousPersonTypes) queryClient.setQueryData(['PersonTypes'], context.previousPersonTypes); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['PersonTypes']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['PersonTypes']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouveau type personne</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier type personne</DialogTitle>
...
function validate(PersonType) {
  return {
    code: !validateRequired(PersonType.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(PersonType.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(PersonType.domaine) ? 'Domaine est requis' : '',
  };
}

  return (
    <div>
      <h2>Liste des Types Personnes</h2>
      <MaterialReactTable table={table} />

      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        <DialogTitle>Confirmer la suppression</DialogTitle>
        <DialogContent>
          Voulez-vous vraiment supprimer le type personne <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
