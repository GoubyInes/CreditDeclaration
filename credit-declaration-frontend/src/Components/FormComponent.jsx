...
const FormComponent = () => {
  ...
  //call READ hook
  const {data: allForms = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Forms'],
  ...
  useEffect(() => {
    if (allForms?.length) {
      setTotalRowCount(allForms.length);
      const paginated = allForms.slice(
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Form) =>{ return await FormService.createForm(Form)},
        onMutate: async (Form) => {
          await queryClient.cancelQueries(['Forms']);
          const previousForms = queryClient.getQueryData(['Forms']);
          queryClient.setQueryData(['Forms'], (prev) => [
            ...(prev || []),
            {...Form},
          ]); 
          return { previousForms };
        },
        onSuccess: () => queryClient.invalidateQueries(['Forms']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await FormService.updateForm(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Forms'], (prev) =>
        prev?.map((prev) =>
  ...
  function useDelete() {
    ...
    return useMutation({
      ...
      onError: (err, code, context) => {if (context?.previousForms) queryClient.setQueryData(['Forms'], context.previousForms); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Forms']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Forms']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle forme</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier forme</DialogTitle>
...
function validate(Form) {
  return {
    code: !validateRequired(Form.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(Form.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(Form.domaine) ? 'Domaine est requis' : '',
  };
}
  ...
  return (
    <div>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          Voulez-vous vraiment supprimer la forme <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
