...
const SituationComponent = () => {
  const [validationErrors, setValidationErrors] = useState({});
  const [data, setData] = useState([]);
  //call READ hook
  const {data: allSituations = [], sError: isLoadingDataError,isFetching: isFetchingData,isLoading: isLoadingData} = useQuery({
    queryKey: ['Situations'],
    queryFn: SituationService.getSituations,
  });
  ...
  useEffect(() => {
    if (allSituations?.length) {
      setTotalRowCount(allSituations.length);
      const paginated = allSituations.slice(
        pagination.pageIndex * pagination.pageSize,
        (pagination.pageIndex + 1) * pagination.pageSize
      );
      setData(paginated);
    }
  }, [allSituations, pagination]);
  ...
  function useCreate() {
     ...
     return useMutation({
        mutationFn :  async(Situation) =>{ return await SituationService.createSituation(Situation)},
        onMutate: async (Situation) => {
          await queryClient.cancelQueries(['Situations']);
          const previousSituations = queryClient.getQueryData(['Situations']);
          queryClient.setQueryData(['Situations'], (prev) => [
            ...(prev || []),
            {...Situation},
          ]); 
          return { previousSituations };
        },
        onSuccess: () => queryClient.invalidateQueries(['Situations']), //refrech data
    });
  }
  ...
  function useUpdate() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({oldCode, data }) => {await SituationService.updateSituation(oldCode, data )},
      onMutate: (newInfo) => {
      queryClient.setQueryData(['Situations'], (prev) =>
        prev?.map((prev) =>
      ...
      onSuccess: () => queryClient.invalidateQueries(['Situations']),
    });
  }
  ...
  function useDelete() {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: SituationService.deleteSituation,
      onMutate: async (code) => {
        queryClient.setQueryData(['Situations'], (prev) =>
          prev?.filter((p) => p.code !== code)
      )},
      onError: (err, code, context) => {if (context?.previousSituations) queryClient.setQueryData(['Situations'], context.previousSituations); },//rollback
      onSettled: () => {queryClient.invalidateQueries(['Situations']);}, // 🔄 Refetch depuis le serveur
      onSuccess: () => queryClient.invalidateQueries(['Situations']),
    });
  }
  //CREATE action
  ...
  const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Créer nouvelle situation</DialogTitle>
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h4">Modifier situation</DialogTitle>
...
function validate(Situation) {
  return {
    code: !validateRequired(Situation.code)
      ? 'Code est requis'
      : '',
    descriptif: !validateRequired(Situation.descriptif) ? 'Descriptif est requis' : '',
    domaine: !validateRequired(Situation.domaine) ? 'Domaine est requis' : '',
  };
}

  return (
    <div>
      <h2>Liste des Situations Crédit</h2>
      <MaterialReactTable table={table} />

      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        <DialogTitle>Confirmer la suppression</DialogTitle>
        <DialogContent>
          Voulez-vous vraiment supprimer la situation <strong>{selectedRowToDelete?.original?.descriptif}</strong> ?
        </DialogContent>
        <DialogActions>
